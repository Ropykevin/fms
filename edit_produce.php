...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (
        empty($report_id) || empty($animal_id) || empty($report_date) ||
        empty($produce_type) || empty($quantity)
    ) {
    ...
    // Update the produce report
    $query = "UPDATE produce_report 
    ...
    if (mysqli_query($con, $query)) {
        ...
        $response['message'] = "Produce report updated successfully";
    } else {
        $response['message'] = "Error updating produce report: " . mysqli_error($con);
    }
} else {
